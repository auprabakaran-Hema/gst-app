...
def write_annual_reconciliation(client_dir, client_name, gstin, log):
    ...
    def read_gstr1(month_name, year):
        ...
        try:
            ...
            for r in d.get("b2cs",[]):
                ...
                b2cs_inv_val = tv + ig + cg + sg  # Invoice value = taxable + tax
                t["b2cs_tx"]+=tv; t["igst"]+=ig; t["cgst"]+=cg; t["sgst"]+=sg
                t["val"]+=b2cs_inv_val  # FIX: add B2CS invoice value to total val
                rt_b2cs=r.get("rt",0)
                key_r={0:"rate_0",3:"rate_3",5:"rate_5",12:"rate_12",
                       18:"rate_18",28:"rate_28"}.get(rt_b2cs,"rate_other")
                t[key_r]+=tv
                rows.append(("B2CS","","(Small Consumers)",
                             "-","-", round(b2cs_inv_val,2),
    ...
    srow("TOTAL Taxable Turnover (B2B+B2CL+B2CS+Exp+Nil+NonGST)", "",
         [round(g1[f"{mn}_{yr}"]["b2b_tx"]+g1[f"{mn}_{yr}"]["b2cs_tx"]
                +g1[f"{mn}_{yr}"].get("b2cl_tx",0)+g1[f"{mn}_{yr}"].get("exp_tx",0)
    ...
    ws2.freeze_panes = "A3"
    ri2=3
    ann2={"inv":0,"b2b":0.,"b2cs":0.,"nil_ex":0.,"non_gst":0.,"ig":0.,"cg":0.,"sg":0.}
    for mn,_,yr in MONTHS:
        mk=f"{mn}_{yr}"; d=g1[mk]
        bg2=ALT2 if ri2%2==0 else ALT1
        nil_ex=round(d.get("nil_exempt",0),2); non_gst=round(d.get("non_gst",0),2)
        tx=round(d["b2b_tx"]+d["b2cs_tx"]+nil_ex+non_gst,2); tt=round(d["igst"]+d["cgst"]+d["sgst"],2)
        for ci2,v in enumerate([f"{mn} {yr}",d["inv"],round(d["b2b_tx"],2),round(d["b2cs_tx"],2),
                                 nil_ex,non_gst,
                                 tx,round(d["igst"],2),round(d["cgst"],2),round(d["sgst"],2),
                                 tt],1):
        ...
        ann2["nil_ex"]+=nil_ex; ann2["non_gst"]+=non_gst
        ann2["ig"]+=d["igst"]; ann2["cg"]+=d["cgst"]; ann2["sg"]+=d["sgst"]
    totrow(ws2,ri2,["ANNUAL TOTAL",_fsum("B",3,ri2-1),_fsum("C",3,ri2-1),_fsum("D",3,ri2-1),
                    ...
                    _fsum("H",3,ri2-1),_fsum("I",3,ri2-1),_fsum("J",3,ri2-1),
                    f"=H{ri2}+I{ri2}+J{ri2}"])
    ...
    ann2x={k:0. for k in ["r0","r3","r5","r12","r18","r28","ro","nil_ex","non_gst","tx","ig","cg","sg"]}
    for mn,_,yr in MONTHS:
        mk=f"{mn}_{yr}"; d=g1[mk]
        ...
        nil_ex=round(d.get("nil_exempt",0),2); non_gst=round(d.get("non_gst",0),2)
        tx=round(d["b2b_tx"]+d["b2cs_tx"]+nil_ex+non_gst,2)
        ig=round(d["igst"],2); cg=round(d["cgst"],2); sg=round(d["sgst"],2)
        for ci2x,v in enumerate([f"{mn} {yr}",r0,r3,r5,r12,r18,r28,ro,nil_ex,non_gst,tx,ig,cg,sg,round(ig+cg+sg,2)],1):
        ...
        for k,v in [("r0",r0),("r3",r3),("r5",r5),("r12",r12),
                    ("r18",r18),("r28",r28),("ro",ro),("nil_ex",nil_ex),("non_gst",non_gst),("tx",tx),
                    ("ig",ig),("cg",cg),("sg",sg)]:
            ann2x[k]+=v
    totrow(ws2x,ri2x,["ANNUAL TOTAL",_fsum("B",3,ri2x-1),
                       round(ann2x["r0"],2),round(ann2x["r3"],2),round(ann2x["r5"],2),
                       round(ann2x["r12"],2),round(ann2x["r18"],2),round(ann2x["r28"],2),
                       round(ann2x["ro"],2),round(ann2x["nil_ex"],2),round(ann2x["non_gst"],2),
                       round(ann2x["tx"],2),
    ...
    # ====================================================
    # SHEET: GSTR1_Bill_Wise
    # One row per invoice across all months, fully flat/filterable
    # Columns: Month | Type | GSTIN | Name | Invoice No | Date |
    #          Place of Supply | Rate% | Taxable ₹ | IGST | CGST | SGST | Total Tax
    # NIL/EXEMPT rows show supply description in Invoice No column
    # ====================================================
    ws_bw = wb.create_sheet("GSTR1_Bill_Wise")
    ws_bw.sheet_view.showGridLines = False
    cols_bw = [("Month",12),("Type",10),("GSTIN/UIN",22),("Party Name",28),
               ("Invoice No",18),("Invoice Date",13),("Place of Supply",16),
               ("Rate %",8),("Taxable ₹",16),("IGST ₹",13),("CGST ₹",13),("SGST ₹",13),
               ("Total Tax ₹",14)]
    title(ws_bw, f"GSTR-1 Bill-Wise Detail (All Invoices) — {client_name} ({gstin}) — FY {FY_LABEL}", len(cols_bw))
    hdr(ws_bw, cols_bw)
    ws_bw.freeze_panes = "A3"
    ri_bw = 3
    ann_bw = {"tx":0.,"ig":0.,"cg":0.,"sg":0.}

    for mn,_,yr in MONTHS:
        mk = f"{mn}_{yr}"
        inv_rows_bw = g1_inv_rows.get(mk, [])
        d_bw = g1[mk]
        nil_v  = round(d_bw.get("nil_exempt",0), 2)
        ngs_v  = round(d_bw.get("non_gst",0), 2)
        if not inv_rows_bw and nil_v == 0 and ngs_v == 0:
            continue
        for row_data in inv_rows_bw:
            inv_type,gstin_r,nm,inum,idt,iv,pos,rate,tv,ig,cg,sg = row_data
            bg_bw = ALT2 if ri_bw%2==0 else ALT1
            tt_bw = round(float(ig or 0)+float(cg or 0)+float(sg or 0), 2)
            for ci_bw,v in enumerate([f"{mn} {yr}",inv_type,gstin_r,nm,inum,idt,
                                       pos,rate,tv,ig,cg,sg,tt_bw],1):
                cell(ws_bw,ri_bw,ci_bw,v,bg_bw,
                     numfmt=NUM_FMT if ci_bw in (9,10,11,12,13) else None,
                     align="right" if ci_bw in (8,9,10,11,12,13) else "left")
            ws_bw.row_dimensions[ri_bw].height=14; ri_bw+=1
            ann_bw["tx"]+=float(tv or 0); ann_bw["ig"]+=float(ig or 0)
            ann_bw["cg"]+=float(cg or 0); ann_bw["sg"]+=float(sg or 0)
        # Add NIL/EXEMPT row if present
        if nil_v:
            bg_bw = "FFF2CC"
            for ci_bw,v in enumerate([f"{mn} {yr}","NIL/EXEMPT","",f"Nil/Exempt Supplies",
                                       "—","—","33",0,nil_v,0,0,0,0],1):
                cell(ws_bw,ri_bw,ci_bw,v,bg_bw,
                     numfmt=NUM_FMT if ci_bw in (9,10,11,12,13) else None,
                     align="right" if ci_bw in (8,9,10,11,12,13) else "left")
            ws_bw.row_dimensions[ri_bw].height=14; ri_bw+=1
            ann_bw["tx"]+=nil_v
        # Add Non-GST row if present
        if ngs_v:
            bg_bw = "F2F2F2"
            for ci_bw,v in enumerate([f"{mn} {yr}","NON-GST","",f"Non-GST Supplies",
                                       "—","—","33",0,ngs_v,0,0,0,0],1):
                cell(ws_bw,ri_bw,ci_bw,v,bg_bw,
                     numfmt=NUM_FMT if ci_bw in (9,10,11,12,13) else None,
                     align="right" if ci_bw in (8,9,10,11,12,13) else "left")
            ws_bw.row_dimensions[ri_bw].height=14; ri_bw+=1
    ...
    _bw_count = ri_bw - 3
    totrow(ws_bw, ri_bw, ["ANNUAL TOTAL","","",f"Total: {_bw_count} records",
                            "","","","",
                            round(ann_bw["tx"],2),round(ann_bw["ig"],2),
                            round(ann_bw["cg"],2),round(ann_bw["sg"],2),
                            round(ann_bw["ig"]+ann_bw["cg"]+ann_bw["sg"],2)])
    ws_bw.sheet_properties.tabColor = "375623"
    ...
    for mn,_,yr in MONTHS:
        mk=f"{mn}_{yr}"; d=g1[mk]
        r1tx=round(d["b2b_tx"]+d["b2cs_tx"]
                  +d.get("b2cl_tx",0)+d.get("exp_tx",0)
                  +d.get("nil_exempt",0)+d.get("non_gst",0),2)
    ...
    for mn,_,yr in MONTHS:
        ...
        b2cl_tx=round(d.get("b2cl_tx",0),2); exp_tx_v=round(d.get("exp_tx",0),2)
        nil_tx=round(d.get("nil_exempt",0),2); non_gst_tx=round(d.get("non_gst",0),2)
        # Sum CDN credit/debit from totals
        cdn_cr=round(d.get("cdn_cr",0),2); cdn_dr=round(d.get("cdn_dr",0),2)
        r1_tx=round(b2b_tx+b2cs_tx+b2cl_tx+exp_tx_v+nil_tx+non_gst_tx,2)
        ...
        wr8("3.1(a) B2CS Supplies",    b2cs_tx, 0,
            ...
            0,0,0,0, bg="E2EFDA")
        wr8("3.1(b) Zero Rated (B2CL+Exp)", b2cl_tx+exp_tx_v,0,0,0, 0,0,0,0, bg=ALT2)
        wr8("3.1(c) Nil/Exempt",       nil_tx,0,0,0, 0,0,0,0, bg=ALT2)
        wr8("3.1(e) Non-GST",          non_gst_tx,0,0,0, 0,0,0,0, bg=ALT2)
        ...
        ann8["b2cl_tx"]=ann8.get("b2cl_tx",0)+b2cl_tx; ann8["exp_tx"]=ann8.get("exp_tx",0)+exp_tx_v
        ann8["nil_tx"]=ann8.get("nil_tx",0)+nil_tx; ann8["non_gst_tx"]=ann8.get("non_gst_tx",0)+non_gst_tx
    ...
    ann_r1tx=round(ann8["b2b_tx"]+ann8["b2cs_tx"]+ann8.get("b2cl_tx",0)+ann8.get("exp_tx",0)+ann8.get("nil_tx",0)+ann8.get("non_gst_tx",0),2)
    ...
    for mn,_,yr in MONTHS:
        ...
        r1tx=round(d1["b2b_tx"]+d1["b2cs_tx"]
                  +d1.get("b2cl_tx",0)+d1.get("exp_tx",0)
                  +d1.get("nil_exempt",0)+d1.get("non_gst",0),2)
    ...
    # Determine which months actually have invoice data
    active_months = [(mn,lbl,yr) for mn,lbl,yr in MONTHS
                     if g1_inv_rows.get(f"{mn}_{yr}")]
    month_short_active = [mn[:3]+"-"+yr[2:] for mn,_,yr in active_months]
    n_vals=5  # Taxable, IGST, CGST, SGST, Total Tax
    total_cols=3+len(active_months)*n_vals+n_vals  # GSTIN+Name+Type + active months + FYtotal
    ...
    # Row 2: month group headers (only active months)
    col_s=4
    for ml in month_short_active:
    ...
    for ci3,h3 in enumerate(["GSTIN","Company Name","Type"],1):
        ...
        hc.alignment=_aln("center"); hc.border=_bdr()
    col_s=4
    for _mi in range(len(active_months)+1):
        bg_s="C00000" if _mi==len(active_months) else HDR_BG
    ...
    # Aggregate invoice data by company and month (only active months)
    comp_data={}
    for mn,_,yr in active_months:
        mk=f"{mn}_{yr}"
        for r in g1_inv_rows.get(mk,[]):
            ...
            if key not in comp_data:
                comp_data[key]={"name":nm or gstin_name_map.get(key,""),
                                "type":inv_type,
                                "months":{f"{mm}_{yy}":{"tx":0.,"ig":0.,"cg":0.,"sg":0.}
                                          for mm,__,yy in active_months}}
    ...
    grand_m={f"{mn}_{yr}":{"tx":0.,"ig":0.,"cg":0.,"sg":0.} for mn,_,yr in active_months}
    ...
    for comp_key in sorted_comp:
        ...
        # One row per month (active months only)
        for mn,_,yr in active_months:
...
def process_client(client, base_dir, log, returns_todo=None, existing_driver=None):
    ...
    # Folder = "CompanyName_GSTIN" so both name and GSTIN are visible at a glance
    _gstin_tag = (client.get("gstin","") or "").strip().upper()
    safe = name.replace(" ","_").replace("/","_")
    safe = f"{safe}_{_gstin_tag}" if _gstin_tag else safe
...
def ask_returns_menu():
    ...
    GUI LAUNCHER: if GST_MENU_CHOICE env var is set, auto-select that option.
    """
    _auto = os.environ.get("GST_MENU_CHOICE", "").strip()
    if _auto:
        print(f"  [AUTO] GST menu choice: {_auto}")
    ...
    while True:
        choice = (_auto if _auto else input("  Enter choice: ").strip().upper())
        _auto = ""   # only use env var once
...
def ask_fy_range():
    ...
    _auto_fy = os.environ.get("GST_MENU_FY", "").strip()
    if _auto_fy:
        print(f"  [AUTO] GST FY: {_auto_fy}")
    while True:
        raw = (_auto_fy if _auto_fy else input("  Your selection: ").strip())
        _auto_fy = ""   # use only once
...
def main():
    ...
    _auto_start = os.environ.get("GST_MENU_CHOICE", "").strip()
    _yes_answer = "YES" if _auto_start else input("\n  Type YES to start: ").strip().upper()
    if _yes_answer != "YES":
        print("  Cancelled."); return
    ...
    if len(fy_list) == 1:
        ...
        for i, client in enumerate(clients, 1):
            ...
            try:
                ...
                _gstin_mfy = (client.get("gstin","") or "").strip().upper()
                first_safe= cname.replace(" ","_").replace("/","_")
                first_safe = f"{first_safe}_{_gstin_mfy}" if _gstin_mfy else first_safe
