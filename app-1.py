...
def _auto_download(job_id, gstin, client_name,
    ...
    try:
        ...
        for idx, (month_name, month_num, year) in enumerate(MONTHS_LIST):
            ...
            if "GSTR3B" in returns_set:
                ...
                try:
                    ...
                    if (dl_dir / save_name).exists() or (out_dir / save_name).exists():
                        log(f"── {month_name} {year}: GSTR-3B already downloaded ✓", "ok")
                        triggered[f"{key}_GSTR3B"] = "OK"
                        existing = out_dir / save_name
                        if not existing.exists():
                            _shutil.copy2(str(dl_dir / save_name), str(existing))
                        ...
                        if _click_tile_download("GSTR3B"):
                            ...
                            save_failure_screenshot(f"GSTR3B {month_name} {year} — Tile Not Found")
                except Exception as e:
            ...
            if "GSTR1" in returns_set:
                try:
                    ...
                    if _click_tile_download("GSTR1"):
                        ...
                        save_failure_screenshot(f"GSTR1 {month_name} {year} — Tile Not Found")
                except Exception as e:
            ...
            if "GSTR1A" in returns_set:
                try:
                    ...
                    if _click_tile_download("GSTR1A"):
                        ...
                        save_failure_screenshot(f"GSTR1A {month_name} {year} — Tile Not Found")
                except Exception as e:
            ...
            # ── GSTR-2B: Click tile → Click GENERATE EXCEL → Instant file (same as GSTR-3B) ──
            # CONFIRMED PORTAL FLOW:
            #   Tile DOWNLOAD click  →  Generate Excel page loads
            #   Click GENERATE EXCEL →  file downloads INSTANTLY to browser folder
            #   No link appears. No polling of page needed. Pure folder watch.
            if "GSTR2B" in returns_set:
                save_name = f"GSTR2B_{month_name}_{year}.xlsx"
                try:
                    log(f"\n── {month_name} {year}: GSTR-2B (tile→Generate→instant file) ──")
                    _go_to_dashboard()
                    _select_and_search(month_name)
                    current_tile[0] = "GSTR2B"

                    if _click_tile_download("GSTR2B"):
                        # Generate page loads — snapshot BEFORE clicking Generate
                        time.sleep(0.8)
                        snap = _snap_dl_dir({".xlsx", ".zip", ".json"})

                        # Click GENERATE EXCEL — triggers instant browser download
                        gen_clicked = _try_click(GENERATE_EXCEL_XP, timeout=8)
                        if gen_clicked:
                            log(f"  ✅ GENERATE EXCEL clicked for {month_name} — watching folder...")
                        else:
                            log(f"  ⚠ GENERATE EXCEL not found for {month_name}", "warn")
                            save_failure_screenshot(f"GSTR2B {month_name} {year} — Generate Excel button not found")

                        # Fast-poll folder every 0.5s — file arrives instantly (same as GSTR-3B)
                        ...
                        if new_f:
                            ...
                            save_failure_screenshot(f"GSTR2B {month_name} {year} — File not received within 60s after Generate click")
                    else:
                        triggered[f"{key}_GSTR2B"] = "TILE_FAIL"
                        save_failure_screenshot(f"GSTR2B {month_name} {year} — Tile DOWNLOAD not found on dashboard")
                except Exception as e:
            ...
            # ── GSTR-2A: trigger GENERATE EXCEL (Phase 1 only) ───────────
            if "GSTR2A" in returns_set:
                try:
                    log(f"\n── {month_name} {year}: GSTR-2A (trigger Generate Excel) ──")
                    _go_to_dashboard()
                    _select_and_search(month_name)
                    ...
                    if _click_tile_download("GSTR2A"):
                        ...
                        if _try_click(GENERATE_EXCEL_XP, timeout=8):
                            ...
                            save_failure_screenshot(f"GSTR2A {month_name} {year} — Generate Excel Not Found")
                    else:
                        triggered[f"{key}_GSTR2A"] = "TILE_FAIL"
                        save_failure_screenshot(f"GSTR2A {month_name} {year} — Tile Not Found")
                except Exception as e:
        ...
        if need_phase2:
            ...
            log("  Starting Phase 2 downloads...")
            time.sleep(1)
