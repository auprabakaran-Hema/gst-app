...
==========================================================================
FIX 9 (v3.15) — F3 NATIVE UI PATH (FIX 8's Alt+K is unreliable).
  FIX 8's approach (Escape → Alt+K → S) fails because after Escape closes F3,
  focus isn't on Gateway: Alt+K opens Master Creation, not Company menu.
  NEW: Stay in F3 dialog, use Tab to navigate to "Select Company" button.
  Then in that dialog, type search + Down + Enter (same as before).
  Simpler, more reliable, needs just Tab navigation inside the F3 dialog.

FIX 8 (v3.13) — Remove ALL OCR/mouse from Select Company navigation.
  [SUPERSEDED — see FIX 9] Attempted: Escape → Alt+K → S pure keyboard approach.
  Was meant to avoid OCR but revealed focus management issue post-Escape.
...
def tally_ui_open_company(target_name, already_loaded=False):
    ...
    pyautogui.press("f3")
    time.sleep(2.5)      # wait for F3 Change Company dialog to fully open (was 1.4)
    ...
    # KEYBOARD-ONLY WITHIN F3 DIALOG — no Alt+K which fails due to focus loss.
    #
    # FIX 9: Stay in the F3 Change Company dialog and use Tab to navigate to
    # "Select Company" button. This is more reliable than Escape→Alt+K→S because
    # focus stays within the dialog context the entire time.
    #
    # Path: F3 (already open) → Tab to "Select Company" button → Enter

    # ── Open Select Company dialog via Tab from within F3 ────────────────────
    # The F3 Change Company dialog has buttons at the top/bottom:
    #   [Change Company] [Select Company] [View Report]
    # We Tab to reach "Select Company" (usually 1-2 tabs depending on focus state)
    print("  [UI] Tab → Select Company button (within F3 dialog)...")
    for _ in range(2):
        pyautogui.press("tab")
        time.sleep(0.3)
    
    # Press Enter to activate "Select Company" button
    pyautogui.press("enter")
    time.sleep(2.5)             # wait for Select Company dialog to fully open (increased from 2.0)
